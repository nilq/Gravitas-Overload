...
public class PointClickPlacementTool : EditorWindow {
	...
	[MenuItem("GameObject/Point N' Click")]
	...
	private void AddSingle (Vector3 buildPos, RaycastHit clickedObject) {
		...
		Vector3 newPos = new Vector3 (Mathf.RoundToInt (buildPos.x), Mathf.RoundToInt (buildPos.y), 0);
	...
	private void WarnUser () {
		...
		Debug.Log("Le shit works only with prefabs!");
	}
	...
	void OnGUI() {
		...
		if (enableHelp == false) {
			...
			EditorGUILayout.HelpBox ("Click the 'Enable Help' butt'n to get help!", MessageType.Info);
		}
		...
		if (enabled == false) {
			...
			if (enableHelp) {
				...
				EditorGUILayout.HelpBox ("Click le 'Enable' butt'n to enable le tool!", MessageType.Info);
				
				EditorGUILayout.Space ();
				EditorGUILayout.Space ();
				
				EditorGUILayout.HelpBox ("You'll need at least 1 collider!", MessageType.Info);
